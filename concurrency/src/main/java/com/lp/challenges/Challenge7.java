...
class Transfer implements Runnable {
    ...
    public void run() {
        while (!sourceAccount.transfer(destinationAccount, amount))
            continue;
        System.out.format("%s completed transferring %f from %s to %s%n", Thread.currentThread().getName(),
                amount, sourceAccount.getAccountNumber(), destinationAccount.getAccountNumber());
    }
}
